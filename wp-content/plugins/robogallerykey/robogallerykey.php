...
define('ROBO_GALLERY_KEY_VERSION', '1.5'); 
...
if (!class_exists('roboGalleryParent')) {
	...
	class roboGalleryParent{
		...
		public function getTagsMenu($class='', $style=''){
			...
			$menuTagSort = get_post_meta( $this->id,  ROBO_GALLERY_PREFIX.'menuTagSort', true );
			
			if(!is_array($this->selectImages->tags)) return '';

			if($menuTagSort=='asc'){
				asort($this->selectImages->tags);	
			}
			if($menuTagSort=='desc'){
				arsort($this->selectImages->tags);	
			}

			foreach ($this->selectImages->tags as $key => $val) {
				$retHtml .= '<a href="#" data-filter=".tag_id'.$key.'"'
						.' class="button '.$class.'"'
						.' '.($style?'style="'.$style.'"':'')
					.'>'.esc_attr($val).'</a>';
			}
